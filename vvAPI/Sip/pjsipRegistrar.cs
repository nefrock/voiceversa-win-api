...
namespace Sipek.Sip
{
  public class pjsipRegistrar : IRegistrar
  {
    ...
    [DllImport(PJSIP_DLL, CallingConvention = CallingConvention.Cdecl, EntryPoint = "dll_registerAccount")]
    ...
    [DllImport(PJSIP_DLL, CallingConvention = CallingConvention.Cdecl, EntryPoint = "dll_removeAccounts")]
    private static extern int dll_removeAccounts();
    [DllImportAttribute(PJSIP_DLL, CallingConvention = CallingConvention.Cdecl, EntryPoint = "onRegStateCallback")]
    ...
    public override int registerAccounts()
    {
      ...
      for (int i = 0; i < Config.Accounts.Count; i++)
      {
        ...
        if (acc.Enabled && (acc.Id.Length > 0)&&(acc.HostName.Length > 0))
        {
          ...
          string uri = schema + ":" + acc.UserName;
          if (acc.UserName.IndexOf("@") < 0)
          {
            uri += "@" + acc.HostName;
          }
          string reguri = schema + ":" + acc.HostName;
